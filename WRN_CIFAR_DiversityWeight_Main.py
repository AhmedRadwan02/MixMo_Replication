...
def parse_args():
    parser = argparse.ArgumentParser(description='Train MixMo models for Figure 8 (reweighting parameter experiment)')
    ...
    parser.add_argument('--mix_prob', type=float, default=0.5, help='Fixed mixing probability (p=0.5 for Figure 8)')
    parser.add_argument('--data_root', type=str, default='/home/pariamdz/projects/def-hinat/pariamdz/MixMo/datasets')
    parser.add_argument('--save_dir', type=str, default='/home/pariamdz/projects/def-hinat/pariamdz/MixMo/results/figure8')
    parser.add_argument('--run_number', type=int, default=1, help='Run 1, 2, or 3 for averaging')
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--r_values', type=str, default='1,2,4,6,8,10,12', help='Comma-separated list of r values to test')
...
def mixmo_loss(outputs, targets, kappa=None, r=3.0):
    """MixMo loss weighting scheme from equation (3) with variable r parameter"""
    ...
    if kappa is not None:
        # Apply weighting function from equation (3) with varying r parameter
...
def train_epoch(model, train_loader, optimizer, device, args, r_param, epoch=0, total_epochs=None):
    """Unified training function for all approaches with controllable r parameter"""
    model.train()
    running_loss = 0.0
    mixing_prob =0.5  # Fixed mixing probability for Figure 8 (typically 0.5)
    ...
    for inputs, targets in tqdm(train_loader, desc="Training"):
        ...
        # Calculate loss with the specific r parameter
        loss = mixmo_loss((out1, out2, out_mix1, out_mix2), (y1, y2), kappa, r=r_param)
...
def train_model_with_r_param(args, r_param, data_handler, device, num_classes):
    """Train a model with a specific r parameter and evaluate it"""
    print(f"\n===== Training with reweighting parameter r = {r_param:.1f} =====")
    ...
    exp_name = f"{args.approach}_r{r_param:.1f}_b{batch_repetitions}_wrn{args.width}_{args.dataset}_run{args.run_number}"
    ...
    for epoch in range(args.epochs):
        ...
        # Train the model with the specific r parameter
        train_loss, train_acc, train_acc1, train_acc2 = train_epoch(
            model, train_loader, optimizer, device, args, 
            r_param=r_param, epoch=epoch, total_epochs=args.epochs
        )
        scheduler.step()
        ...
        # For Figure 8, we want to collect final results, especially from the last 10 epochs
    ...
    # Save detailed results for this r parameter
...
def main():
    ...
    # Figure 8 experiment: vary r parameter and measure performance
    # Parse r values from command line argument
    r_values = [float(r) for r in args.r_values.split(',')]
    figure8_results = []
    
    for r_param in r_values:
        # Set seed based on run number and reset for each r parameter
        seed = args.seed + args.run_number - 1
        set_seed(seed)
        
        # Train and evaluate model with this r parameter
        avg_metrics = train_model_with_r_param(
            args, r_param, data_handler, device, num_classes
        )
        
        figure8_results.append(avg_metrics)
    
    # Save the combined results for Figure 8
    figure8_path = os.path.join(args.save_dir, f"figure8_results_{args.approach}_run{args.run_number}.json")
    with open(figure8_path, 'w') as f:
        json.dump(figure8_results, f, indent=4)
    
    print(f"Figure 8 experiment completed. Results saved to {figure8_path}")
    
    # Print summary of results
    print("\nSummary of results for different r parameters:")
    print("r\tEnsemble Acc\tIndividual Acc\tDiversity")
    for result in figure8_results:
        print(f"{result['r']:.1f}\t{result['ensemble_acc']:.2f}%\t{result['individual_acc']:.2f}%\t{result['diversity_dre']:.4f}")
